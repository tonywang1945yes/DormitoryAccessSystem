...
public class Mail {
	 ...
	 */
	public static void sendSimpleMail(String sendAddress, String password, String receiveAddress, String content)
			throws GeneralSecurityException {
		try {
			...
			message.setSubject("This is a test.");// 主题
	...
	public static void sendMailWithAttachment(String sendAddress, String password, String receiveAddress,
			String content, String attachmentPath) throws GeneralSecurityException {
		try {
			MimeMessage message = new MimeMessage(init(sendAddress, password));
			message.setFrom(new InternetAddress(sendAddress));// 发件人
			message.addRecipient(Message.RecipientType.TO, new InternetAddress(receiveAddress));// 收件人
	 ...
	 * @throws GeneralSecurityException
	 */
	private static Session init(final String sendAddress, final String password) throws GeneralSecurityException {
	 ...
	 */
	private static String toZip(File directory, String fileName) {
		if (!directory.exists()) {
